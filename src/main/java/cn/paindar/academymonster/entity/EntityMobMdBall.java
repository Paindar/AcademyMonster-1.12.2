 ...
 */
@RegEntity
public class EntityMobMdBall extends EntityAdvanced {

    private static final DataParameter<Integer> SPAWNER_ID = EntityDataManager.createKey(EntityMobMdBall.class, DataSerializers.VARINT);
    private static final DataParameter<Float> SUB_X = EntityDataManager.createKey(EntityMobMdBall.class, DataSerializers.FLOAT);
    private static final DataParameter<Float> SUB_Y = EntityDataManager.createKey(EntityMobMdBall.class, DataSerializers.FLOAT);
    private static final DataParameter<Float> SUB_Z = EntityDataManager.createKey(EntityMobMdBall.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> LIFE = EntityDataManager.createKey(EntityMobMdBall.class, DataSerializers.VARINT);
    ...
    public EntityMobMdBall(Entity spawner, int life, final EntityCallback<EntityMobMdBall> callback) {
        ...
        // Calc the sub-offset
        float theta = -spawner.rotationYaw / 180 * MathUtils.PI_F +
    ...
    private float getSize() {
        double lifeMS = life * 0.05;
        double time = GameTimer.getTime();
        double dt = time - spawnTime;

        if(dt > lifeMS - 0.1)
            return Math.max(0, MathUtils.lerpf(1.5f, 0, (float) ((dt - (lifeMS - 0.1)) / 0.1)));
        if(dt > lifeMS - 0.3)
            return MathUtils.lerpf(1, 1.5f, (float) ((dt - (lifeMS - 0.3)) / 0.2));
        return 1;
    }
    ...
    @RegEntityRender(EntityMobMdBall.class)
    @SideOnly(Side.CLIENT)
    public static class R extends RenderIcon<EntityMobMdBall> {
        ...
        @Override
        public void doRender(EntityMobMdBall ent, double x, double y,
